...
| `px_hex_replace_title` | 把选中的色票换成这个颜色（整张图用到它的格子一起变） | 把選中的色票換成這個顏色（整張圖用到它的格子一起變） | 選択中の色をこの色に置き換え（使っているマスも一緒に変わります） | Replace the selected swatch with this colour (every cell using it changes too) | Reemplazar la muestra seleccionada por este color (todas las celdas que la usan cambian también) |
...
| `toast_wm_conflict` | 文字与图片水印位置相同，请各自选择不同位置 | 文字與圖片浮水印位置相同，請各自選擇不同位置 | テキストと画像の透かし位置が同じです。それぞれ異なる位置を選択してください | Text and image watermarks share the same position — please choose different positions | Las marcas de agua de texto e imagen comparten posición — elige posiciones distintas |
...
| `batch_cancel_all` | 全部取消 | 取消全部 | すべてキャンセル | Cancel all | Cancelar todo |
| `batch_remove` | 从列表移除 | 從清單移除 | リストから削除 | Remove from list | Quitar de la lista |
| `batch_dismiss` | 忽略提示 | 忽略提示 | 閉じる | Dismiss | Descartar |
