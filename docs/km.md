...
## 20 · 讀 code 推不出鍵位，實機才算數（2026-09-16）

**事件**。編輯器 19 個工具的切換鍵，手冊一律標 `⇧` 加字母（`⇧V`／`⇧O`⋯⋯）。
Design 席依 Tauri 席的判讀提出改成裸鍵：理由是 `editor.js` 的 switch 寫著
`case 'o': case 'O':`，看起來兩種都收；且 app 的工具標籤五語一致都印裸鍵
（`疊入圖片 (O)`、`選取 (V)`）。Nova 實機測後：

```
⇧O            → 開      Caps Lock + O → 開      小寫 o → 不開
```

判準是 `e.key` 是否為大寫 `O`，與用什麼手段打出大寫無關。`case 'o'` 那一支
實測觸發不到。**手冊原樣正確，改下去會讓四顆鍵位在十個檔、五個語系同時變錯。**

**為什麼值得記**。同一個提案出現過兩次——Design 自己的交接包 README 才剛
警告過「不要改成裸鍵，Nova 實機測過」，隔一輪又因為重讀 code 而提出來。
**一條靠記憶維持的結論會被下一次重讀推翻；寫進 km 才擋得住第三次。**

**可複用的判準**：
- **原始碼分支存在 ≠ 那條分支走得到。** 前置判斷、事件攔截、IME 都可能讓它變死碼。
- **app 的標籤不是鍵位的證據。** 反例當場就有：`隱私遮蔽 (K)` 印裸鍵，實際要 `⇧K`。
  同一份證據在 K 身上已被證偽，就不能拿去證明 C／S／E／O。
- **風險單向時，要求證據的一方是對的。** 當時三種可能裡，「只收裸鍵」已被
  既有測試排除，於是改動只可能「一樣對」或「變錯」，沒有上行空間。
  沒有上行空間的改動，不值得用一次實機測試以外的任何理由推進。
- **部分套用比全套更危險。** 該提案只改 4 顆、留 15 顆帶 `⇧`，讀者會推論
  那四顆是特例。要嘛全改要嘛全不改，否則手冊得多一句話解釋為什麼不一樣。
