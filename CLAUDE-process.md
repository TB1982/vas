...
├── tools/gen_sitemap.py    # sitemap 產生器（CI 於 PR 階段自動跑；本機 python3 tools/gen_sitemap.py）
│
...
| `sitemap.xml` | AUTO-GENERATED (104 URLs). Never hand-edit — CI runs `tools/gen_sitemap.py` during the PR (commits into the branch), lastmod from git. New page → add a line to `sitemap.manifest.json` |
